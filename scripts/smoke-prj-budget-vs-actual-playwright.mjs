...
const OUTPUT_DIR = path.resolve('artifacts', 'playwright-prj-budget-vs-actual');
...
function loadAccounts() {
  const json = process.env.PRJ_BUDGET_VS_ACTUAL_SMOKE_ACCOUNTS_JSON;
  if (json && String(json).trim()) {
    const parsed = JSON.parse(json);
    if (!Array.isArray(parsed) || parsed.length === 0) {
      throw new Error('PRJ_BUDGET_VS_ACTUAL_SMOKE_ACCOUNTS_JSON must be a non-empty array');
    }

    return parsed.map((account, index) => {
      if (!account?.email || !account?.password) {
        throw new Error(`PRJ_BUDGET_VS_ACTUAL_SMOKE_ACCOUNTS_JSON entry ${index} must include email and password`);
      }
  ...
  const fallbackAccounts = [
    {
      key: 'FR',
      email:
        process.env.PRJ_BUDGET_VS_ACTUAL_SMOKE_FR_EMAIL || process.env.PILOTAGE_FR_EMAIL || 'pilotage.fr.demo@cashpilot.cloud',
      password: requirePassword('PRJ_BUDGET_VS_ACTUAL_SMOKE_FR_PASSWORD', 'PILOTAGE_FR_PASSWORD'),
    },
    {
      key: 'BE',
      email:
        process.env.PRJ_BUDGET_VS_ACTUAL_SMOKE_BE_EMAIL || process.env.PILOTAGE_BE_EMAIL || 'pilotage.be.demo@cashpilot.cloud',
      password: requirePassword('PRJ_BUDGET_VS_ACTUAL_SMOKE_BE_PASSWORD', 'PILOTAGE_BE_PASSWORD'),
    },
    {
      key: 'OHADA',
      email:
        process.env.PRJ_BUDGET_VS_ACTUAL_SMOKE_OHADA_EMAIL ||
        process.env.PILOTAGE_OHADA_EMAIL ||
        'pilotage.ohada.demo@cashpilot.cloud',
      password: requirePassword('PRJ_BUDGET_VS_ACTUAL_SMOKE_OHADA_PASSWORD', 'PILOTAGE_OHADA_PASSWORD'),
    },
  ];
  ...
  if (accounts.length === 0) {
    throw new Error(
      'No PRJ-02 smoke accounts configured. Set PRJ_BUDGET_VS_ACTUAL_SMOKE_ACCOUNTS_JSON or demo password env vars.'
    );
  }
...
async function runAccount(page, account) {
  ...
  try {
    ...
    const profitabilityTab = page.getByRole('tab', { name: /Rentabilité|Profitability|projects\.profitability\.title/i }).first();
    await profitabilityTab.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
    await profitabilityTab.click();
    result.checks.push('profitability-tab-opened');

    const panel = page.getByTestId('project-budget-vs-actual-panel').first();
    await panel.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
    await page.getByText(/Rentabilité budget vs réel/i).first().waitFor({
      state: 'visible',
      timeout: DEFAULT_TIMEOUT,
    });
    result.checks.push('budget-vs-actual-panel-visible');

    const hasBudgetCards = await page.getByText(/Budget CA/i).first().isVisible().catch(() => false);
    if (hasBudgetCards) {
      await page.getByText(/Écart marge/i).first().waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
      result.checks.push('budget-vs-actual-cards-visible');
    } else {
      await page.getByText(/Budget incomplet/i).first().waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
      result.checks.push('budget-vs-actual-fallback-visible');
    }
