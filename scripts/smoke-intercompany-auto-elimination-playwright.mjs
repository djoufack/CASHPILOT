...
const OUTPUT_DIR = path.resolve('artifacts', 'playwright-intercompany-auto-elimination');
...
async function openInterCompany(page) {
  await page.goto(`${BASE_URL}/app/inter-company`, { waitUntil: 'domcontentloaded' });
  await page.waitForLoadState('networkidle', { timeout: DEFAULT_TIMEOUT }).catch(() => {});
...
async function runAccount(page, account) {
  ...
  try {
    ...
    await openInterCompany(page);

    const autoButton = page.getByTestId('intercompany-auto-elimination-button').first();
    await autoButton.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });

    const buttonBefore = await autoButton.innerText();
    await autoButton.click();
    await page.waitForTimeout(1500);

    const buttonAfter = await autoButton.innerText();
    result.checks.push({
      key: 'auto_button_click',
      before: buttonBefore.trim(),
      after: buttonAfter.trim(),
    });

    const eliminationTab = page.getByRole('button', { name: /elim/i }).first();
    await eliminationTab.click();
    await page.waitForLoadState('networkidle', { timeout: DEFAULT_TIMEOUT }).catch(() => {});

    result.checks.push({
      key: 'elimination_tab_opened',
      url: page.url(),
    });
