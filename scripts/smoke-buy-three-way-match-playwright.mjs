...
const OUTPUT_DIR = path.resolve('artifacts', 'playwright-buy-three-way-match');
...
function loadAccounts() {
  const json = process.env.BUY_THREE_WAY_SMOKE_ACCOUNTS_JSON;
  if (json && String(json).trim()) {
    const parsed = JSON.parse(json);
    if (!Array.isArray(parsed) || parsed.length === 0) {
      throw new Error('BUY_THREE_WAY_SMOKE_ACCOUNTS_JSON must be a non-empty array');
    }

    return parsed.map((account, index) => {
      if (!account?.email || !account?.password) {
        throw new Error(`BUY_THREE_WAY_SMOKE_ACCOUNTS_JSON entry ${index} must include email and password`);
      }
  ...
  const fallbackAccounts = [
    {
      key: 'FR',
      email: process.env.BUY_THREE_WAY_SMOKE_FR_EMAIL || process.env.PILOTAGE_FR_EMAIL || 'pilotage.fr.demo@cashpilot.cloud',
      password: requirePassword('BUY_THREE_WAY_SMOKE_FR_PASSWORD', 'PILOTAGE_FR_PASSWORD'),
    },
    {
      key: 'BE',
      email: process.env.BUY_THREE_WAY_SMOKE_BE_EMAIL || process.env.PILOTAGE_BE_EMAIL || 'pilotage.be.demo@cashpilot.cloud',
      password: requirePassword('BUY_THREE_WAY_SMOKE_BE_PASSWORD', 'PILOTAGE_BE_PASSWORD'),
    },
    {
      key: 'OHADA',
      email:
        process.env.BUY_THREE_WAY_SMOKE_OHADA_EMAIL ||
        process.env.PILOTAGE_OHADA_EMAIL ||
        'pilotage.ohada.demo@cashpilot.cloud',
      password: requirePassword('BUY_THREE_WAY_SMOKE_OHADA_PASSWORD', 'PILOTAGE_OHADA_PASSWORD'),
    },
  ];
  ...
  if (accounts.length === 0) {
    throw new Error(
      'No BUY 3-way smoke accounts configured. Set BUY_THREE_WAY_SMOKE_ACCOUNTS_JSON or demo password env vars.'
    );
  }
...
async function runAccount(page, account) {
  ...
  try {
    ...
    await page.getByText(/3-way|three-way|3 way/i).first().waitFor({
      state: 'visible',
      timeout: DEFAULT_TIMEOUT,
    });
    result.checks.push('three-way-column-visible');

    const badge = await firstVisible([
      page.getByText(/3-way OK|Ecart|Partiel|En attente|Non lie|Matched|Mismatch|Partial|Unmatched/i).first(),
    ]);
    if (badge) {
      result.checks.push('three-way-badge-visible');
    }

    const recalcButton = await firstVisible([
      page.locator("button[title*='3-way']").first(),
      page.getByRole('button', { name: /Recalc|Recalcul|Recalculate/i }).first(),
    ]);
    if (recalcButton) {
      await recalcButton.click();
      await page.waitForLoadState('networkidle', { timeout: DEFAULT_TIMEOUT }).catch(() => {});
      result.checks.push('three-way-recalculate-clicked');
    } else {
      result.checks.push('three-way-recalculate-not-required');
    }
