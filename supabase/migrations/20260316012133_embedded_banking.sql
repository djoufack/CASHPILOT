...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'bank_providers' AND policyname = 'Anyone can read bank_providers') THEN
    CREATE POLICY "Anyone can read bank_providers" ON bank_providers FOR SELECT USING (auth.uid() IS NOT NULL);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'bank_account_connections' AND policyname = 'Users manage their bank_account_connections') THEN
    CREATE POLICY "Users manage their bank_account_connections" ON bank_account_connections FOR ALL
      USING (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_account_connections.company_id AND c.user_id = auth.uid()))
      WITH CHECK (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_account_connections.company_id AND c.user_id = auth.uid()));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'bank_sync_logs' AND policyname = 'Users manage their bank_sync_logs') THEN
    CREATE POLICY "Users manage their bank_sync_logs" ON bank_sync_logs FOR ALL
      USING (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_sync_logs.company_id AND c.user_id = auth.uid()))
      WITH CHECK (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_sync_logs.company_id AND c.user_id = auth.uid()));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'bank_transfers' AND policyname = 'Users manage their bank_transfers') THEN
    CREATE POLICY "Users manage their bank_transfers" ON bank_transfers FOR ALL
      USING (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_transfers.company_id AND c.user_id = auth.uid()))
      WITH CHECK (EXISTS (SELECT 1 FROM public.company c WHERE c.id = bank_transfers.company_id AND c.user_id = auth.uid()));
  END IF;
END $$;
...
INSERT INTO bank_providers (name, api_type, base_url, supported_countries, logo_url, is_active, config) VALUES
  ('Plaid', 'plaid', 'https://production.plaid.com', ARRAY['US','CA','GB','FR','ES','NL','IE'], 'https://plaid.com/assets/img/logos/plaid-logo.svg', true, '{"version":"2020-09-14","products":["transactions","auth","balance","identity"]}'::jsonb),
  ('Nordigen (GoCardless)', 'nordigen', 'https://bankaccountdata.gocardless.com/api/v2', ARRAY['FR','DE','BE','NL','ES','IT','PT','AT','IE','FI','SE','NO','DK','PL','CZ','LT','LV','EE','HU','RO','BG','HR','SK','SI','LU','MT','CY','GR','GB'], 'https://gocardless.com/assets/images/brand/gocardless-mark.svg', true, '{"access_valid_for_days":90,"max_historical_days":730,"access_scope":["balances","details","transactions"]}'::jsonb),
  ('Bridge (Bankin)', 'bridge', 'https://api.bridgeapi.io/v2', ARRAY['FR','ES','DE','GB'], 'https://bridgeapi.io/images/bridge-logo.svg', true, '{"version":"2021-06-01"}'::jsonb),
  ('Tink', 'tink', 'https://api.tink.com/api/v1', ARRAY['SE','FI','NO','DK','DE','AT','NL','BE','FR','ES','IT','PT','GB','IE'], 'https://cdn.tink.se/assets/tink-logo.svg', true, '{"market":"EU"}'::jsonb)
