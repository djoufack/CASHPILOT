...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'consent_logs' AND policyname = 'consent_logs_select_own') THEN
    CREATE POLICY consent_logs_select_own ON consent_logs FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'consent_logs' AND policyname = 'consent_logs_insert_own') THEN
    CREATE POLICY consent_logs_insert_own ON consent_logs FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'consent_logs' AND policyname = 'consent_logs_update_own') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'data_export_requests' AND policyname = 'data_export_select_own') THEN
    CREATE POLICY data_export_select_own ON data_export_requests FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'data_export_requests' AND policyname = 'data_export_insert_own') THEN
