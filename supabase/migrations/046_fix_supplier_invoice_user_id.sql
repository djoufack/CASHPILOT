-- 046: Fix supplier_invoices user_id resolution
-- supplier_invoices does not have a user_id column.
-- Ownership is resolved via supplier_id -> suppliers.user_id.
-- This migration updates all functions that previously referenced supplier_invoices.user_id.

-- 1. Fix auto_journal_supplier_invoice trigger function
...
-- 2. Fix reverse_journal_supplier_invoice trigger function
...
-- 3. Fix backfill_accounting_entries to join through suppliers for supplier_invoices
