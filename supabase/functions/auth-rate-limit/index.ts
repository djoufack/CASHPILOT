...
const normalizeIdentifier = (value: unknown) => String(value || 'global').trim().toLowerCase().slice(0, 320);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (outcome === 'attempt') {
      ...
      if (!row?.allowed) {
        ...
        const retryAfterSeconds = Math.max(
          1,
          Math.ceil(((resetAt?.getTime() ?? Date.now()) - Date.now()) / 1000),
        );
        return jsonResponse(
          {
    ...
    if (outcome === 'success') {
      ...
      return jsonResponse({ recorded: true, action: 'success' });
    }
    ...
    const lockSeconds = nextFailedAttempts >= HARD_LOCK_THRESHOLD
      ? Math.min(MAX_LOCK_SECONDS, BASE_LOCK_SECONDS * (2 ** escalationStep))
      : 0;
    const lockUntil = lockSeconds > 0
