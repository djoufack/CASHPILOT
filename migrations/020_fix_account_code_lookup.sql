-- ============================================================================
-- Migration 020: Fix get_user_account_code() custom mapping lookup
-- ============================================================================
-- BUG: When trigger calls get_user_account_code(user_id, 'expense.office'),
-- the custom mapping lookup uses source_category='general' (default param)
-- instead of extracting 'office' from the dotted key. This causes:
--   1. Wrong mapping row matched (expense.general instead of expense.office)
--   2. Returns credit_account_code (bank) instead of debit_account_code (expense)
--   3. All expense entries get debited to bank account instead of proper expense account
--
-- FIX: Extract source_category from dotted key + always return debit_account_code
-- ============================================================================
...
BEGIN
  ...
  -- For dotted keys like 'expense.office', extract source_category from the key
  -- Always return debit_account_code (the primary account for that mapping)
