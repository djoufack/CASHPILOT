...
SI une ou plusieurs taches sont FAIL :
   a. Pour chaque tache FAIL :
      ...
      - Relire la specification de la tache dans task-to-do/
      - Relancer le sous-agent avec le feedback precis d'erreur
      - Attendre completion
...
4. CONSTRUIRE un nouveau prompt corrige contenant :
   ...
   - Le contenu ACTUEL du/des fichier(s) a modifier (pas une version imaginee)
...
9. Si OUI → marquer la tache comme completee, passer a la suivante
...
Si la verification retourne des taches en FAIL :
    Pour chaque tache FAIL :
        1. Lire le detail de l'echec (quel critere non respecte)
        2. Relire la specification de la tache dans task-to-do/
        3. Relancer le sous-agent avec le feedback precis
        4. Re-verifier la tache individuellement
