...
${entriesXml}
  ...
  server.tool(
    ...
    async ({ invoice_id, profile }) => {
      ...
      const fxCompanyQuery = supabase.from('company').select('*').eq('user_id', getUserId());
      if (inv.company_id) fxCompanyQuery.eq('id', inv.company_id);
      const companyRes = await fxCompanyQuery.limit(1).single();
...
<rsm:CrossIndustryInvoice
  ...
  server.tool(
    ...
    async ({ invoice_id, type }) => {
      ...
      // Fetch seller company scoped to the invoice's company_id (supports multi-company users)
      const ublCompanyQuery = supabase.from('company').select('*').eq('user_id', getUserId());
      if (inv.company_id) ublCompanyQuery.eq('id', inv.company_id);
      const companyRes = await ublCompanyQuery.limit(1).single();
      const seller = companyRes.data || {};
