  ...
  const content = `
    <div style="font-family: Arial, sans-serif; max-width: 1000px; margin: 0 auto; padding: 20px;">
      ...
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #374151; text-align: center; color: #6b7280; font-size: 12px;">
        <p style="margin: 0;">Analytics Report generated by CashPilot - ${new Date().toLocaleString(getLocale())}</p>
      </div>
    </div>
  `;
  ...
  const content = `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
      ...
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb; text-align: center; color: #6b7280; font-size: 12px;">
        <p style="margin: 0;">Supplier Report generated by CashPilot - ${new Date().toLocaleString(getLocale())}</p>
      </div>
    </div>
  `;
  ...
  const content = `
    <div style="font-family: Arial, sans-serif; max-width: 1000px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%); color: white; padding: 30px; border-radius: 8px; margin-bottom: 30px;">
        ...
        <p style="margin: 0;">${companyInfo?.name || 'Your Company'} - ${new Date().toLocaleDateString(getLocale())}</p>
      </div>
...
export const exportDashboardPDF = async (data, companyInfo) => {
  ...
  const { metrics, recentInvoices, recentTimesheets: _recentTimesheets } = data;
  ...
  const content = `
    <div style="font-family: Arial, sans-serif; max-width: 1000px; margin: 0 auto; padding: 20px;">
      ...
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #374151; text-align: center; color: #6b7280; font-size: 12px;">
        <p style="margin: 0;">Dashboard Report generated by CashPilot - ${new Date().toLocaleString(getLocale())}</p>
      </div>
    </div>
  `;
...
export const exportDashboardHTML = (data, companyInfo) => {
  const { metrics, revenueData, clientRevenueData: _clientRevenueData, recentInvoices, recentTimesheets } = data;
            ...
            <tbody>
              ${recentTimesheets
                ...
                <tr style="border-bottom: 1px solid #374151;">
                  ...
                  <td style="padding: 10px; text-align: center; color: #f3f4f6;">${new Date(ts.date).toLocaleDateString(getLocale())}</td>
                </tr>
              `
                )
                .join('')}
