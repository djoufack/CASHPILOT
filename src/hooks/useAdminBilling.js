...
export const useAdminBilling = () => {
  ...
  const fetchBillingData = useCallback(async () => {
    ...
    try {
      ...
      const creditsRes = _results[2].status === 'fulfilled' ? _results[2].value : { data: null, error: null };
      const plansRes = _results[3].status === 'fulfilled' ? _results[3].value : { data: null, error: null };

      if (profileRes.error) throw profileRes.error;
      if (creditsRes.error) throw creditsRes.error;
      if (plansRes.error) throw plansRes.error;
      if (roleRes.error) {
        console.warn('Admin role lookup skipped:', roleRes.error.message);
      }
      ...
      const allUserIds = new Set([...profilesByUserId.keys(), ...rolesByUserId.keys(), ...creditsByUserId.keys()]);
      ...
      const nextRecords = [...allUserIds].map((userId) => {
        ...
        return {
          ...
          access_role: rolesByUserId.get(userId) || normalizeRole(profile?.role),
