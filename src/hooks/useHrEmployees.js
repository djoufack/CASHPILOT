...
/**
 * Hook responsible for HR employees, employee contracts, and
 * synchronisation between hr_employees and team_members.
 */
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      _results.forEach((r, i) => {
        if (r.status === 'rejected') console.error(`HrEmployees fetch ${i} failed:`, r.reason);
      });
      ...
      if (employeesResult.error) console.error('HrEmployees query error:', employeesResult.error);
      if (employeeContractsResult.error)
        console.error('HrEmployeeContracts query error:', employeeContractsResult.error);
