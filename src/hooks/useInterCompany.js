...
export function useInterCompany() {
  ...
  const computeEliminations = useCallback(
    async (periodStart, periodEnd, portfolioId) => {
      ...
      const { data, error } = await supabase.from('intercompany_eliminations').insert([row]).select('*').single();
      ...
      toast({
        title: 'Eliminations calculees',
        description: `${entriesCount} ecritures eliminees pour un total de ${totalEliminated.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} EUR.`,
      });
