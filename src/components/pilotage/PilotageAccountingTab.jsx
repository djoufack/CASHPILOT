    ...
    <motion.div className="space-y-8" variants={containerVariants} initial="hidden" animate="visible">
      ...
      <motion.div variants={itemVariants}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-bold text-gray-100">{t('pilotage.ratios.structure')}</h2>
        </div>
        {availability?.structure?.status === 'unavailable' ? null : (
      ...
      <motion.div variants={itemVariants}>
        {availability?.liquidity?.status === 'unavailable' ? null : (
      ...
      <motion.div variants={itemVariants}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-bold text-gray-100">{t('pilotage.ratios.activity')}</h2>
        </div>
        {availability?.activity?.status === 'unavailable' ? null : (
      ...
      <motion.div variants={itemVariants}>
        {availability?.trialBalance?.status === 'unavailable' ? null : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-800/50">
                      {displayedRows.map((row, idx) => (
                        <tr key={row.account_code || idx} className="hover:bg-gray-800/30 transition-colors">
                          <td className="py-2 px-3 text-gray-300">
                            <span className="font-mono text-xs text-gray-500 mr-2">{row.account_code}</span>
                            {row.account_name}
                          </td>
