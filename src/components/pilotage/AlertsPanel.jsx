...
import { AlertTriangle, AlertCircle, CheckCircle, XCircle, Bell, ArrowUpRight } from 'lucide-react';
...
const ALERTS_INFO = {
  ...
  calculationMethod:
    'Les alertes sont triées par sévérité décroissante puis affichées avec leur message et leurs seuils.',
    ...
    <Card className="bg-gray-900/50 border border-gray-800/50 rounded-xl h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-gray-200 flex items-center gap-2">
          ...
          {hasAlerts && <span className="ml-auto text-xs font-normal text-gray-500">{sortedAlerts.length}</span>}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {hasAlerts ? (
          <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
            {sortedAlerts.map((alert, index) => (
              <AlertRow key={index} alert={alert} t={t} action={resolveAlertAction(alert)} />
            ))}
          </div>
        ) : (
