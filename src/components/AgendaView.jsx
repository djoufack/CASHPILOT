...
const AgendaGroup = ({ title, tasks, onEdit, onDelete, onOpenTask }) => {
  ...
  return (
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tasks.map(task => (
           <TaskCard key={task.id} task={task} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
        ))}
      </div>
    </div>
  );
...
const AgendaView = ({ tasks, onEdit, onDelete, onOpenTask }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <AgendaGroup title={t('tasks.agenda.overdue')} tasks={overdueTasks} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
      <AgendaGroup title={t('tasks.agenda.today')} tasks={todayTasks} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
      <AgendaGroup title={t('tasks.agenda.tomorrow')} tasks={tomorrowTasks} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
      <AgendaGroup title={t('tasks.agenda.thisWeek')} tasks={thisWeekTasks} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
      <AgendaGroup title={t('tasks.agenda.laterNoDate')} tasks={laterTasks} onEdit={onEdit} onDelete={onDelete} onOpenTask={onOpenTask} />
