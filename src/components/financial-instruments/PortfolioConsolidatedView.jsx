...
export const PortfolioConsolidatedView = ({ data = [], baseCurrency = 'EUR' }) => {
  ...
  const panelInfo = {
    companyTotal: {
      title: t('financialInstruments.baseBalance', `Balance (${baseCurrency})`),
      definition: 'Total de la société exprimé dans la devise du portefeuille.',
      dataSource: 'Soldes consolidés des instruments de la société.',
      formula: 'Total société = somme des soldes convertis de ses instruments.',
      calculationMethod: 'Additionne les instruments de la société puis affiche le total en en-tête.',
      expertDataSource: 'RPC Supabase `rpc_portfolio_consolidated_balances`.',
      expertFormula: 'company_total = sum(balance_base_currency) group by company.',
      expertCalculationMethod: 'Agrégation locale `companyTotals` construite via Map.',
    },
    originalBalance: {
      title: t('financialInstruments.originalBalance', 'Balance'),
      definition: "Solde de l'instrument dans sa devise d'origine.",
      dataSource: 'Montant natif de chaque instrument.',
      calculationMethod: 'Affichage direct tel quel.',
      expertDataSource: 'Colonne `balance` (devise instrument).',
      expertCalculationMethod: 'Rendu sans conversion, formaté avec la devise d origine.',
    },
    baseBalance: {
      title: t('financialInstruments.baseBalance', `Balance (${baseCurrency})`),
      definition: 'Même solde, converti dans la devise du portefeuille.',
      dataSource: 'Montant converti pour comparaison entre sociétés.',
      calculationMethod: 'Affiche la valeur convertie utilisée pour la consolidation.',
      expertDataSource: 'Colonne `balance_base_currency`.',
      expertCalculationMethod: 'Valeur de référence utilisée pour tous les totaux consolidés.',
    },
    grandTotal: {
      title: t('financialInstruments.grandTotal', 'Grand Total'),
      definition: 'Total consolidé de toutes les sociétés du portefeuille.',
      dataSource: 'Totaux convertis de toutes les sociétés affichées.',
      formula: 'Grand Total = somme des totaux de chaque société.',
      calculationMethod: 'Addition finale des montants consolidés.',
      expertDataSource: 'Somme de tous les `balance_base_currency` de la vue.',
      expertFormula: 'grand_total = Σ(company_total).',
      expertCalculationMethod: 'Accumulation locale dans `grandTotal`.',
    },
  };
  ...
  return (
    <div className="space-y-4">
      {Array.from(grouped.entries()).map(([companyName, instruments]) => {
        ...
        return (
          <div key={companyName} className="rounded-xl border border-gray-700/50 bg-[#0f1528]/60 backdrop-blur-sm">
            {/* Company header */}
            <div className="flex items-center justify-between border-b border-gray-700/50 px-5 py-3">
              ...
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-500">
                <PanelInfoPopover
                  {...panelInfo.companyTotal}
                  ariaLabel={`Informations sur ${panelInfo.companyTotal.title}`}
            ...
            <div className="divide-y divide-gray-800/50">
              ...
              <div className="grid grid-cols-12 gap-2 px-5 py-2 text-xs font-medium uppercase tracking-wider text-gray-500">
                ...
                <div className="col-span-4">{t('financialInstruments.instrument', 'Instrument')}</div>
                <div className="col-span-2 text-center">{t('financialInstruments.type', 'Type')}</div>
                <div className="col-span-2 text-right">
                  <span className="inline-flex items-center justify-end gap-1.5">
                    <PanelInfoPopover
                      {...panelInfo.originalBalance}
                      ariaLabel={`Informations sur ${panelInfo.originalBalance.title}`}
                      triggerClassName="h-5 w-5"
                    />
                ...
                <div className="col-span-3 text-right">
                  <span className="inline-flex items-center justify-end gap-1.5">
                    <PanelInfoPopover
                      {...panelInfo.baseBalance}
                      ariaLabel={`Informations sur ${panelInfo.baseBalance.title}`}
                      triggerClassName="h-5 w-5"
                    />
                  ...
                  >
                    ...
                    <div className="col-span-4">
                      ...
                      {instrument.account_number && (
                        <p className="truncate text-xs text-gray-500">{instrument.account_number}</p>
                      )}
                    </div>
                    ...
                    <div className="col-span-2 text-right">
                      <span
                        className={`text-sm font-medium ${originalBalance >= 0 ? 'text-gray-300' : 'text-red-400'}`}
                      >
                    ...
                    <div className="col-span-3 text-right">
                      <span className={`text-sm font-semibold ${baseBalance >= 0 ? 'text-gray-200' : 'text-red-400'}`}>
      ...
      {grouped.size > 1 && (
        <div className="flex items-center justify-between rounded-xl border border-amber-500/20 bg-amber-500/5 px-5 py-3 backdrop-blur-sm">
          <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-200">
            <PanelInfoPopover
              {...panelInfo.grandTotal}
              ariaLabel={`Informations sur ${panelInfo.grandTotal.title}`}
              triggerClassName="h-5 w-5"
            />
            {t('financialInstruments.grandTotal', 'Grand Total')}
          </span>
          <span className={`text-base font-bold ${grandTotal >= 0 ? 'text-amber-500' : 'text-red-400'}`}>
