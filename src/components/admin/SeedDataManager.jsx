  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">{t('seedDataManager.title')}</h1>
        <p className="text-muted-foreground">
          {t('seedDataManager.subtitle')}
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('seedDataManager.testUsers')}
            </CardTitle>
            <CardDescription>
              {t('seedDataManager.testUsersDescription')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="rounded-md bg-muted p-4 text-sm space-y-2">
                <p className="font-medium">{t('seedDataManager.accountsToCreate')}</p>
                <ul className="list-disc pl-4 space-y-1 text-muted-foreground">
                  <li>scte.test@cashpilot.cloud (Role: User)</li>
                  <li>freelance.test@cashpilot.cloud (Role: User)</li>
                </ul>
                <p className="text-xs text-muted-foreground">
                  {t('seedDataManager.generatedPasswords')}
                </p>
                <p className="text-xs text-muted-foreground">
                  {t('seedDataManager.adminAccessNote')}
                </p>
              </div>
              <Button 
              ...
              >
                ...
                {t('seedDataManager.createTestUsers')}
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('seedDataManager.seedDatabase')}
            </CardTitle>
            <CardDescription>
              {t('seedDataManager.seedDatabaseDescription')}
            </CardDescription>
          </CardHeader>
          <CardContent>
             <div className="space-y-4">
              <div className="rounded-md bg-muted p-4 text-sm space-y-2">
                <p className="font-medium">{t('seedDataManager.dataToGenerate')}</p>
                <ul className="list-disc pl-4 space-y-1 text-muted-foreground">
                  <li>{t('seedDataManager.suppliersAndLocations')}</li>
                  <li>{t('seedDataManager.productsAndInventory')}</li>
                  <li>{t('seedDataManager.servicesAndPricing')}</li>
                  <li>{t('seedDataManager.notificationsAndAuditLogs')}</li>
                </ul>
              </div>
              <Button 
      ...
      <Card className="h-[400px] flex flex-col">
        <CardHeader>
          <CardTitle>{t('seedDataManager.operationLog')}</CardTitle>
          <CardDescription>{t('seedDataManager.operationLogDescription')}</CardDescription>
        </CardHeader>
        <CardContent className="flex-1 min-h-0">
          <ScrollArea className="h-full w-full rounded-md border p-4 bg-black/5">
            {progress.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                <p>{t('seedDataManager.noOperations')}</p>
              </div>
            ) : (
