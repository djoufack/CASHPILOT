...
const BalanceSheet = ({ balanceSheet, period, currency = 'XAF', companyInfo, onExportPDF, onExportHTML }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* View content */}
      {viewMode === 'summary' ? (
        <SummaryView syscohada={syscohada} totalAssets={totalAssets} totalPassif={totalPassif} fmt={fmt} currency={currency} />
      ) : (
        <DetailedView syscohada={syscohada} totalAssets={totalAssets} totalPassif={totalPassif} fmt={fmt} currency={currency} />
      )}
            ...
            <table className="w-full text-xs" style={{ tableLayout: 'auto' }}>
              ...
              <tbody>
                {rows.map((row, i) => {
                  if (row.type === 'section') {
                    return (
                      <tr key={`s-${i}`} className="bg-gray-800/70 border-t-2 border-gray-600">
                        <td colSpan={4} className="py-2.5 px-1.5 font-bold text-white uppercase tracking-wide text-[11px]">
                          {row.label}
                        </td>
                        <td className="py-2.5 px-1.5 text-right font-bold font-mono text-white">
                          {fmt(row.total)}
                        </td>
                      </tr>
                    );
                  ...
                  if (row.type === 'group') {
                    return (
                      <tr key={`g-${i}`} className="bg-gray-800/30 border-t border-gray-700">
                        ...
                        <td className="py-1.5 px-1.5 font-bold text-orange-400 uppercase text-[11px]">{row.className}</td>
                        <td colSpan={2}></td>
                        <td className="py-1.5 px-1.5 text-right font-mono font-bold text-orange-400">{fmt(row.subtotal)}</td>
                      </tr>
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`py-1 px-1.5 ${isZero ? 'text-gray-600' : 'text-gray-300'}`}>{row.account_name}</td>
                ...
                <tr className={`border-t-2 ${borderColor}`}>
                  ...
                  <td className={`py-2.5 px-1.5 text-right font-bold font-mono text-lg ${colorClass} whitespace-nowrap`}>
                    {fmt(total)}
                  </td>
                </tr>
              </tbody>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg px-4 py-2.5 text-xs text-blue-300 flex items-center gap-2">
        ...
        Vue détaillée — Plan comptable complet avec tous les postes (y compris soldes à 0) pour vérification expert-comptable.
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
