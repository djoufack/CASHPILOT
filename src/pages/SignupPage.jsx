...
const SignupPage = () => {
  ...
  const validateForm = () => {
    ...
    if (password !== confirmPassword)
      newErrors.confirmPassword = t('signup.errors.passwordMismatch', 'Passwords do not match');
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const normalizedEmail = sanitizeText(email).trim().toLowerCase();
      const result = await signUp(normalizedEmail, password, fullName, companyName, role);
      ...
      toast({
        ...
        description: hasSession
          ? t('signup.redirecting', 'Account created! Redirecting...')
          : t(
              'signup.confirmationEmailSent',
      ...
      setTimeout(
        () => {
          navigate(hasSession ? '/app/onboarding' : '/login', { replace: true });
        },
        hasSession ? 1500 : 2000
      );
    } catch (error) {
      console.error(error);
      let errorMessage = error.message;
      if (error.code === 'AUTH_RATE_LIMITED') {
        errorMessage = t('signup.errors.rateLimited', 'Too many attempts. Try again in {{seconds}} seconds.', {
  ...
  return (
    <div className="min-h-[100dvh] min-h-screen bg-gray-950 flex items-center justify-center p-4 relative overflow-x-hidden overflow-y-auto">
      <Helmet>
        <title>{t('signup.title', 'Sign Up')} | CashPilot</title>
      </Helmet>
        ...
        <div className="bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-2xl p-1">
          ...
          <div className="bg-gray-950/90 rounded-xl p-8 relative z-10 h-full">
            <div className="text-center mb-8">
              <h1 className="text-4xl font-bold text-gradient mb-2 tracking-tight">{t('app.name')}</h1>
            ...
            {success ? (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-white">
                  {t('signup.accountCreated', 'Account created successfully')}
                </h3>
                <p className="text-gray-400 text-sm">
                  {requiresEmailConfirmation
                    ? t(
                        'signup.confirmationEmailPending',
                        'We sent a confirmation email. Please verify your inbox, then log in.'
                      )
                    : t('signup.signupSuccess', 'Signup successful, redirecting...')}
                </p>
              </motion.div>
            ) : (
