...
const InvoicesPage = () => {
    ...
    guardedAction(
      ...
      async () => {
        try {
          ...
          captureError(error, { tags: { scope: 'invoices', action: 'export_facturx' }, extra: { invoiceId: invoice.id } });
          toast({ title: t('common.error'), description: error?.message || t('common.unexpectedError', 'An unexpected error occurred.'), variant: 'destructive' });
          throw error;
        }
      }
  ...
  const handleGeneratePaymentLink = async (invoice) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('stripe-invoice-link', { body: { invoiceId: invoice.id } });
      ...
      captureError(err, { tags: { scope: 'invoices', action: 'generate_payment_link' }, extra: { invoiceId: invoice.id } });
      toast({ title: t('common.error'), description: err?.message || t('common.unexpectedError', 'An unexpected error occurred.'), variant: 'destructive' });
    } finally {
  ...
  const invoiceExportColumns = [
    ...
    { key: 'client', header: t('clients.companyName'), width: 25, accessor: (inv) => { const client = clients.find(c => c.id === (inv.client_id || inv.clientId)); return client?.company_name || client?.companyName || ''; }},
  ...
  const getStatusColor = (status) => {
    const colors = { paid: 'bg-green-500', sent: 'bg-blue-500', overdue: 'bg-red-500', cancelled: 'bg-gray-600', default: 'bg-gray-500' };
