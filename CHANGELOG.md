...
- Added `scripts/guard-git-history-secrets.mjs` to detect concrete secret patterns in full Git history.
- Added `test:coverage:enterprise` command with enforced 100% line coverage on enterprise-critical guard/snapshot modules.
- Added `src/utils/displayFormatting.js` to centralize currency/date display formatting with safe fallbacks.
...
- Integrated `guard:git-history-secrets` into the global `npm run guard` pipeline.
- Integrated `test:coverage:enterprise` into the global `npm run guard` pipeline.
- Hardened `npm run lint` with `--max-warnings=0` so warnings fail the pipeline.
...
- Replaced duplicated local date/currency formatting logic in key modules with shared helpers (`AccountantDashboardPage`, `CRMPage`, dunning and employee/supplier views).
