# Task 12 - HAUTE : Edge Function extract-invoice (Gemini API)

## Fichiers a creer
- `supabase/functions/extract-invoice/index.ts` (CREER)

## Probleme
Aucune edge function n'existe pour extraire les donnees structurees d'une facture fournisseur via un LLM.

## Solution
Creer une edge function Deno qui :
1. Recoit { filePath, fileType, userId } en POST
2. Verifie les credits utilisateur via service role
3. Telecharge le fichier depuis Supabase Storage
4. Encode en base64
5. Appelle Gemini 2.0 Flash API pour extraction structuree
6. Deduit les credits (3) et log la transaction
7. Retourne le JSON structure
8. Gere les erreurs (402, 404, 422, 502) et rembourse en cas d'echec Gemini

Pattern a suivre : `supabase/functions/stripe-checkout/index.ts` (memes CORS headers, meme structure Deno serve)
...
serve(async (req) => {
  ...
  try {
    ...
    if (!geminiResponse.ok) {
      // Refund credits on Gemini failure
      await supabase.from('user_credits').update({ balance: creditsData.balance }).eq('user_id', userId);
      await supabase.from('credit_transactions').insert([{
...
- [ ] Fichier `supabase/functions/extract-invoice/index.ts` existe
- [ ] Contient import serve et createClient
- [ ] Contient CORS headers identiques au pattern stripe-checkout
- [ ] Contient verification des credits (402 si insuffisants)
- [ ] Contient download depuis supplier-invoices storage
- [ ] Contient appel Gemini API avec inlineData base64
- [ ] Contient prompt d'extraction structure
- [ ] Contient mecanisme de refund en cas d'echec
- [ ] Contient log dans credit_transactions
