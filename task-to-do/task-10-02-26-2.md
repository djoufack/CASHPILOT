...
## Résultat
- Colonne `expense_date DATE` ajoutée à `expenses`
- Données existantes peuplées via `UPDATE expenses SET expense_date = created_at::date`
- Trigger `auto_journal_expense()` mis à jour : `COALESCE(NEW.expense_date, NEW.created_at::date, CURRENT_DATE)`
- Frontend (`ExpensesPage.jsx`, `useExpenses.js`) mis à jour pour inclure `expense_date`
- Migration locale : `migrations/029_add_expense_date.sql`
